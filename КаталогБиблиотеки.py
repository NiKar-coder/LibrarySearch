...
from PyQt6.QtWidgets import QApplication, QMainWindow, QTableWidgetItem, QPushButton, QWidget, QVBoxLayout, QLabel
...
class Biblio(QMainWindow, Ui_1):
    def __init__(self):
        ...
        self.data = cur.execute("""SELECT Books.title, Authors.name, Books.year, Genres.name, Books.picture FROM Books
    ...
    def searches(self):
        if self.comboBox.currentText() == 'Название':
            books = [i for i in self.data if self.lineEdit.text().lower() in i[0].lower()]
        else:
            books = [i for i in self.data if self.lineEdit.text().lower() in i[1].lower()]
...
class Ui_2(object):
    ...
    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "Информация о произведении"))
